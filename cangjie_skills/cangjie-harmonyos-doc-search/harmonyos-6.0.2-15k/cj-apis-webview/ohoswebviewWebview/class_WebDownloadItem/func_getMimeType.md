...
**功能：** 获取下载的媒体类型。例如，一个声音文件可能被标记为audio/ogg，一个图像文件可能是image/png。
...
|类型|说明|
|:----|:----|
|String|下载的媒体类型（例如，一个声音文件可能被标记为audio/ogg，一个图像文件可能是image/png）。|
                    ...
                    webDownloadDelegate.onBeforeDownload(
                        {
                            ...
                            AppLog.info("will start a download, mime type: ${item.getMimeType()}")
                            // 传入一个下载路径，并开始下载。
