...
public class DHCommonParamsSpec <: AsyKeySpec {
    public init(algName!: String, specType!: AsyKeySpecType, p!: BigInt,
                g!: BigInt, l!: IntNative)
}
```

**功能：** 密钥参数[AsyKeySpec](#interface-asykeyspec)的子类，用于指定DH算法中公私钥包含的参数。
...
|p|BigInt|是|-| **命名参数。** 指定DH算法中大素数p。|
|g|BigInt|是|-| **命名参数。** 指定DH算法中参数g。|
|l|IntNative|是|-| **命名参数。** 指定DH算法中私钥的长度，单位为bit。|