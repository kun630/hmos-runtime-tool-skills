...
class FileElfWrapper : public ElfWrapper {
 public:
  explicit FileElfWrapper(const char *name) : fd_(-1) {
    ...
    if (fd_ < 0) {
      OH_LOG_Print(LOG_APP, LOG_WARN, 0x00008, "FileElfWrapper", "open %s fail, errno %d", name, errno);
      return;
    }

    size_ = lseek(fd_, 0, SEEK_END);
    if (size_ <= 0) {
      OH_LOG_Print(LOG_APP, LOG_WARN, 0x00008, "FileElfWrapper", "lseek fail or size %zu errno %d", size_, errno);
      return;
    }
    ...
    if (start_ == MAP_FAILED) {
      OH_LOG_Print(LOG_APP, LOG_WARN, 0x00008, "FileElfWrapper", "mmap size %zu fail, errno %d", size_, errno);
      return;
    }
  }
...
class MemoryElfWrapper : public ElfWrapper {
 public:
  explicit MemoryElfWrapper(std::string &elf_data) {
    ...
    start_ = const_cast<char *>(elf_data_.data());
