...
T *ElfReader::CheckedOffset(off_t offset, size_t size) {
  if (!IsValidRange(offset + size)) {
    OH_LOG_Print(LOG_APP, LOG_WARN, 0x00008, "ElfReader", "illegal offset %lld, ELF start is %p", offset,
          elf_wrapper_->Start());
    return nullptr;
  }
...
ElfW(Addr) ElfReader::LookupByElfHash(const char *symbol) {
  if (!has_elf_hash_ || !dynsym_ || !dynstr_) {
    OH_LOG_Print(LOG_APP, LOG_WARN, 0x00008, "ElfReader", "ELF Hash miss or check dynsym/dynstr");
    return 0;
  }
