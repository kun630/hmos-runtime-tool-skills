...
|参数名|类型|必填|默认值|说明|
|:---|:---|:---|:---|:---|
|callback|(Float64, Int32, String)->Unit|是|-|回调函数，在删除完成时触发。仅支持系统输入法输入的场景。第一个参数表示删除的值的位置信息。第二个参数表示删除值的方向。第三个参数表示删除的值。|
...
|参数名|类型|必填|默认值|说明|
|:---|:---|:---|:---|:---|
|callback|(Float64, String)->Unit|是|-|回调函数，在输入完成时触发。仅支持系统输入法输入的场景。第一个参数表示插入的值的位置信息。第二个参数表示插入的值。|
