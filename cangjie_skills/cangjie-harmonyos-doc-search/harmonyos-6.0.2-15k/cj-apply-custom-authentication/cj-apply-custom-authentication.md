...
![authentivation-widget.png](./figures/authentivation-widget.png)
...
针对需要切换自定义认证方式的场景，发起认证请求的方式请参见[发起认证](cj-start-authentication.md)，但在传入的widgetParam需包含字段navigationButtonText。

当前示例仅展示如何配置界面、选择切换到自定义认证界面，具体拉起的页面及对应页面请开发者自行实现。
...
try {
  ...
  let authParam = AuthParam(randData, [UserAuthType.FACE], AuthTrustLevel.ATL3)
  // 配置认证界面需设置navigationButtonText
  let widgetParam = WidgetParam('请验证身份', '使用密码')
  // 获取认证对象
  ...
  userAuthInstance.on('result', {
      ...
      if (userAuthResult.result == 12500011) {
