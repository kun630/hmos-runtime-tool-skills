...
支持SDK范围在API 12及以上，设备类型为phone、tablet的ArkTS（加 C++）HAP模块或设备类型为default、phone、tablet的ArkTS（加 C++）HSP/HAR模块中创建仓颉模块。

若当前HAP模块支持的设备类型包括phone、tablet之外的其他设备类型，HSP/HAR模块支持的设备类型包括default、phone、tablet之外的其他设备类型，请将HAP、HSP或HAR模块下module.json5中的deviceTypes改成支持的设备类型格式，再进行创建插入仓颉模块内容的操作。

![deviceTypes](../figures/start-deviceTypes.png)

1. 按照下图所示，选中 ArkTS（加 C++）的HAP、HSP或HAR模块中的任意文件，选择 **File ->
    new -> Cangjie(Interop)**。

    ![enableCangjie](../figures/start-enableCangjie.png)

2. 点击 **Cangjie(Interop)** 按钮后即可完成在 ArkTS（加 C++）的HAP、HSP或HAR模块中创建插入仓颉模块内容。

    ![finishEnableCangjie](../figures/start-finishEnableCangjie.png)
...
3. 在DevEco Studio中打开上述所说的仓颉文件，在文件编辑界面中右键选择**Generate... > Cangjie-ArkTS Interop API**，会在**cangjie->types->libohos_app_cangjie_entry**目录下生成Index.d.ts声明文件和oh_package.json5配置文件，以及会在**cangjie -> ark_interop_api**目录下生成ark_interop_api.d.ts声明文件和oh_package.json5配置文件，并在HAP或HAR模块下的oh-package.json5文件中自动添加依赖：
    ...
    > **说明：**
    >
    > ark_interop_api目录为兼容需要运行在HarmonyOS 5.0.x上的应用生成，如果没有兼容性需求，则可以忽略此目录。
...
> **说明：**
>
> 此加载方式从API version 18开始支持。最低兼容版本：HarmonyOS 5.1.0(18)。如果需要兼容运行在HarmonyOS 5.0.x上的应用，请参考仓颉开发指南的在ArkTS中加载仓颉模块章节内容。
